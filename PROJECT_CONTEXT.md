...
- Attendance Notifications & Weekly Reports:
  - **Students**: No daily emails or push notifications on check-in or check-out. Weekly attendance reports covering the past 7 days are sent exclusively to parents via their registered `parent_email`. Daily absence emails are suppressed on session end in favor of the weekly summary.
  - **Staff**: Daily check-in and check-out events trigger real-time FCM push notifications to all active organization users on their registered devices, with viewers strictly excluded (`User.role != "viewer"`). No daily emails are sent to HR. A consolidated 7-day staff attendance summary is dispatched weekly to `company.hr_email`.
  - **Weekly Dispatch**: Automatic dispatch via Vercel cron (`/attendance/cron/weekly-reports` scheduled weekly), with dedicated `/cron/weekly-parent-reports` and `/cron/weekly-staff-reports` endpoints. Manual on-demand dispatch is available from the Reports page UI (`POST /attendance/reports/send-weekly-reports`).
- A Vercel cron (`/attendance/cron/end-sessions`, once/day on Hobby) ends sessions past their `session_end_time` and, for `check_in` sessions, marks unrecorded students and employees as `absent` (without sending daily absence emails; absences appear in weekly reports). Kiosk queries treat a session as ended immediately once `session_end_time` passes (lazy expiry), so the dashboard reflects it without waiting for the cron. Manual attendance corrections support present, absent, and excused for admin/HR/branch-manager roles; viewers are read-only.
