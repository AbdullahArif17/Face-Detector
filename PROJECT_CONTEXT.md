...
- HuggingFace AI service is deployed at `https://abdullah017-face-attendance-ai.hf.space`; production-hardening commit `07e88ef` is rebuilding as of 2026-07-12. Recheck `/health` after the Space reaches `RUNNING`.
- GitHub production-hardening commit `6275dee` is deployed. Backend `https://face-detector-k4dl.vercel.app` passes `/health`; `/ready` currently reports the database ready but waits for the rebuilding AI Space and a Vercel `BIOMETRIC_ENCRYPTION_KEY`.
...
- Production default WhatsApp credentials and all three attendance template names are configured, but Vercel still needs `META_APP_SECRET` before signed webhook processing and the inbound parent `STATUS` chatbot become ready.
