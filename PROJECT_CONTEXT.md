...
- Attendance Notifications & Weekly Reports:
  ...
  - **Staff**: Daily check-in and check-out events trigger real-time FCM push notifications to organization administrators and managers (`super_admin`, `admin`, `hr`, `branch_manager`) on their mobile devices. No daily emails are sent to HR. A consolidated 7-day staff attendance summary is dispatched weekly to `company.hr_email`.
